...
interface StoreObservers {
  [key: string]: SubscribeCallbakc[];
}
interface SubscribeCallbakc {
  (arg: unknown): void;
}

export class Store<S> {
  public state = {} as S; // Component에서 사용될 속성
  private observers = {} as StoreObservers; // Store 내부에서 사용될 속성
  constructor(state: S) {
  ...
  // 상태 변경 구독!
  subscribe(key: string, cb: SubscribeCallbakc) {
